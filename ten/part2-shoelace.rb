...
class Pos
  ...
  def connected_neighbors(grid)
    ...
    when 'J'
      return [Pos.new(@x,@y-1),Pos.new(@x-1,@y)].select{|p| p.is_valid?(grid)}
    when '7'
      return [Pos.new(@x-1,@y),Pos.new(@x,@y+1)].select{|p| p.is_valid?(grid)}
    when 'F'
      return [Pos.new(@x,@y+1),Pos.new(@x+1,@y)].select{|p| p.is_valid?(grid)}
  ...
  def bafsa(grid)
    ...
    while (curr,prev,depth = stack.pop)
      ...
      if ['F','J','L','7'].include?(curr.grid_get(grid))
        segments << [seg_start,curr] unless seg_start == curr
        seg_start=curr
      end
      if seen[curr]
        puts "cycle! at #{curr}=#{curr.grid_get(grid)}"
        puts pipes.to_s
...
(0...grid.length).each do |y|
  ...
  (0...row.length).each do |x|
    if grid[y][x] == 'S'
      puts "S: #{x},#{y}"
      s = Pos.new(x,y)
      s_neighbors = s.connected_neighbors(grid).sort
      puts "S: #{s} -> #{s_neighbors}"
      die("bad: start has more than 2 connections") unless s_neighbors.length == 2
      if s_neighbors[0].y == s_neighbors[1].y
        puts 's is -'
        grid[y][x] == '-'
      elsif s_neighbors[0].x == s_neighbors[1].x
        puts 's is |'
        grid[y][x] == '|'
      elsif s_neighbors[0].y < s.y # UP
        if s_neighbors[1].x > s.x # RIGHT
          puts 's is L'
          grid[y][x] == 'L'
        elsif s_neighbors[1].x < s.x # RIGHT
          puts 's is J'
          grid[y][x] == 'J'
        else
          die "here; assumption violated"
        end
      elsif s_neighbors[0].x > s.x # RIGHT
        die "assumption violated" unless s_neighbors[1].y > s.y # DOWN
        puts 's is F'
        puts s.grid_get(grid)
        grid[y][x] = 'F'
        puts s.grid_get(grid)
      elsif s_neighbors[0].x < s.x # LEFT
        die "assumption violated" unless s_neighbors[1].y > s.y # DOWN
        puts 's is 7'
        grid[y][x] == '7'
      end
      
      depth,pipes,segments=s.bafsa(grid)
      puts "result:#{depth}"
    end
    # puts "#{x},#{y}"
  end
...
vertices = segments.map{|seg| seg[1]}
